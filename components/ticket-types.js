...
function TicketType({tickets}) {
              ...
              <div className='flex flex-row items-center justify-between'>
                <div className="flex flex-col justify-between">
                   <Image width={30} height={30} 
                     priority 
                     src={idx===0?'/free.png':idx===1?'/gold.png':'/platinum.png'} 
                     alt="tick type" style={{marginBottom:'5px'}}/>
  ...
  export default function TicketTypeGrid() {
    ...
    useEffect(() => {
      if(data){
        setTypes(data.data.reduce((allTicTypes, type) => {
          allTicTypes[type.companyName] = [...(allTicTypes[type.companyName] || []), type];
          return allTicTypes;
        }, {}))
      }
    },[data])
    ...
    return (
      <>
        {Object.keys(types).map((companyName) => (
          <motion.div 
          key={companyName} 
          className={cn(styles.companyRow,
            [styleUtils.appear],
          ...
          >
             <h3 className={cn(styles.title,'absolute -left-6 top-5 -rotate-90')}>Tickets</h3>
